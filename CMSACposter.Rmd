...
1. Before the season begins, would knowing a team's performance during the previous year driveprices? 
2. Would preseason ranking have any influence on prices? 
3. After the start of the season, would rankings, performance, strengthy of schedule, injuries, or other variables affect prices (and volatility)?
...
- Quick visual inspection suggests persistent high confidence for Alabama, Clemson, and Georgia.
...
- The intervals in our option price series are not always equivalent, but we do have many time points for each team. 
- In financial option pricing,volatility is usually a factor of interest.
...
- Are current mean ranking (Mean), wins, injury count (n), and strength of schedule (masseySoS) predictive of volatility?
- Using generalized additive models for our models to be as flexible as possible; all parameters but wins are smoothed.
- Only showing lower level seats here but have similar analysis for upper level.
...
- Strength of schedule and wins did not do much, but mean ranking and number of injuries do contribute to volatility.
- Mean ranking behaves as anticipated (a near linear negative relationship).
- Number of injuries has an interesting relationship with volatility (see the following plots).
...
- Our goal is to be able to use these models next season to buy before a team becomes the hot dark horse and sell become a team's dreams burn in flames.
